...
class MovementController extends MasterController
{
    ...
    public function __construct(MovementModel $model, SingleService $service)
    {
    ...
    {
        $query = $this->model->dataRepository(false)->groupBy(Movement::field_code());
    ...
    public function postMovement(MovementRequest $request, CreateMovementService $service)
    {
        $data = $service->save($this->model, $request);
        return Response::redirectBack($data);
    }
    ...
    public function getUpdate($code)
    {
        ...
        $data = Movement::addSelect([
            'movement.*',
            Asset::field_code(),
            Asset::field_name(),
            Lokasi::field_name(),
        ])
        ->where(Movement::field_code(), $model->field_code)
        ->leftJoinRelationship('has_asset')
        ->leftJoinRelationship('has_from')
        ->get();
    ...
    public function postUpdate($code, Request $request, UpdateMovementService $service)
    {
